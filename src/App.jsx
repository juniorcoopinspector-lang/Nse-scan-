...
export default function App() {
  ...
  const loadStock = useCallback(async (symbol) => {
    try {
      // Fetch daily, intraday, and 15m candles in parallel instead of sequentially —
      // this alone cuts per-stock latency to roughly 1/3.
      const [daily, intraResult, c15Result] = await Promise.all([
        fetchDaily(symbol),
        fetchIntraday(symbol),
        fetch15m(symbol),
      ]);
      const { closes: intraCl, volumes: intraVol } = intraResult;
      const { closes: c15 } = c15Result;
  ...
  // ── Full scan ──────────────────────────────────────────────────────────────
  // Runs stocks in concurrent batches (instead of one-by-one) and paints each
  // result into the table as soon as it lands, instead of waiting for the
  // entire scan to finish before showing anything.
  const BATCH_SIZE = 8; // concurrent requests — high enough to be fast, low enough to avoid tripping the proxy's rate limit
  ...
  const runScan = useCallback(async () => {
    ...
    const symbols = STOCKS.map(decodeURIComponent); // M&M fix
    let done = 0;
    let loadedTotal = 0;

    for (let i = 0; i < symbols.length; i += BATCH_SIZE) {
      const batch = symbols.slice(i, i + BATCH_SIZE);
      setStatusMsg(`Loading ${batch.join(", ")}...`);

      // Fire the whole batch in parallel
      const results = await Promise.all(batch.map(sym => loadStock(sym)));

      // Paint results immediately so the table fills in progressively
      setStocks(prev => {
        const next = { ...prev };
        batch.forEach((sym, idx) => {
          if (results[idx]) next[sym] = results[idx];
        });
        return next;
      });

      done += batch.length;
      loadedTotal += results.filter(Boolean).length;
      setLoadedCount(done);
      setProgress(Math.round((done / symbols.length) * 100));
    }

    setPhase("live");
    setStatusMsg(`Scan complete · ${loadedTotal} stocks loaded`);
